...
do_install_append () {
    ...
    # rename start-stop-daemon to start-stop-daemon-ext
    find ${D} -type f -name "*start-stop-daemon*" -exec mv {} {}-ext \; 

    # support for buggy init.d scripts that refer to an alternative
    # explicit path to start-stop-daemon
    if [ "${base_sbindir}" != "${sbindir}" ]; then
        mkdir -p ${D}${base_sbindir}
        ln -sf ${sbindir}/start-stop-daemon-ext ${D}${base_sbindir}/start-stop-daemon-ext
    fi
}
